...
class fer2013():
    ...
    def kaggle_split(self):
        trainingPixels = self.pixels[self.Usage == 'Training']
        trainingTargets = self.targets[self.Usage == 'Training']

        pubPixels = self.pixels[self.Usage == 'PublicTest']
        pubTargets = self.targets[self.Usage == 'PublicTest']

        privPixels = self.pixels[self.Usage == 'PrivateTest']
        privTargets = self.targets[self.Usage == 'PrivateTest']

        return (trainingPixels,trainingTargets),(pubPixels,pubTargets),(privPixels,privTargets)
...
if __name__ == '__main__':
    ...
    plt.close()
    plt.figure()
    plt.imshow(data.pixels[0,:,:,0])
    total= 0
    for ii in np.linspace(0,6,7):
        count = np.sum(data.emotion == ii)
        total += count
        print(data.label_key(ii) +': ' +  str(count))
    print(total/len(data.emotion))
    print(total)
#    plt.close('all')
#    for ii in range(2):
